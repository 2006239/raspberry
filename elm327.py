...
def aja():
    global window, connection, gpslukeminen, acceleroloop, elm327, kirjoittaminen, event
...
def aloita_lopeta():
    if button['text'] == "Aloita":
        thread1 = threading.Thread(target=aja, daemon=True)
        thread1.start()
        thread2 = threading.Thread(target=paivita, daemon=True)
        thread2.start()
