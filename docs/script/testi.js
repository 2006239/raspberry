...
function parseXML(data) {
  ...
  cycle = this.xml.getElementsByTagName("cycle");
  ...
  for (let i = 0; i <cycle.length; i++) {
     ...
     if (temp != null) {
        var korkeus = parseFloat(temp.textContent);
     ...
     if(i+1==cycle.length)
     {
        ...
        kaavio = new Chart(chart, nopeus(raja));
...
function accelerometer() {
    var kiihtyvyys,x,y,z,sijainnit;
    var dataset = [];
    var gvoima = 0;
    var suurin = 0;
    var temp = 0;
	...
	y = sijainnit.qyuerySelector("y");
        z = sijainnit.querySelector("z");

        if (z != null && y != null && x !=null) {
            gvoima = Math.sqrt(Math.pow(parseFloat(x),2) + Math.pow(parseFloat(y),2) +Math.pow(parseFloat(z),2));
